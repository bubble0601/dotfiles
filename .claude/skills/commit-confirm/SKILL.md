...
2. **AskUserQuestion tool** で「この内容で commit してよいですか?」と確認する。選択肢の例:
   - 「commit する」(Recommended)
   - 「修正したい」(除外・追加・メッセージ変更などは Other で自由入力してもらう)

   テキストで聞いて返答を待つ形にせず、必ず AskUserQuestion を使う。沈黙を承認とみなさない
3. 変更の除外・追加の指示があれば反映して再提示(再度 AskUserQuestion で確認)
