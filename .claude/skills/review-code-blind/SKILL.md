...
>    - 仕様追加・変更が含まれるなら、**仕様面のみを日本語で書く** (ユーザー/業務から見て何ができるようになった・変わったか)。実装詳細 (クラス名・メソッド名・ファイル構成・同梱されたリファクタ・依存更新・migration の中身等) は要点に含めない (それらは「見るべきポイント」側で扱う)。
>    - 純粋に技術的変更のみ (ユーザー挙動が一切変わらない) であれば design レベル (アーキテクチャ / モジュール境界 / データモデル) で書く。
> 3. **レビューで特に見るべきポイント** (3–6 件): この PR の **「性格を表す箇所」「論点になる設計判断・特殊な実装・他と扱いが違う箇所」**を抽象度高めにハイライト。findings (具体疑義) ではなく、レビュワーが diff を読む前に PR の全体像を掴むためのもの。
>    - **拾う対象**: 新しい抽象 / モジュール境界の導入、既存パターンと意図的に違う扱い、設計上の trade-off (パフォーマンス vs 可読性、防御の二段目を入れる/省く等)、業務ルールの解釈が反映されている箇所、既存の不変条件が変わる箇所 (状態遷移追加・API contract 変更)、影響範囲が広い変更の中核 / 「本丸」、技術的に難度が高い実装が含まれる箇所 (並行処理 / 複雑な型 / 独自アルゴリズム / 複雑な SQL 等は「ここに難所がある」事実だけ抽象的に示す)、セキュリティ境界に触れている箇所 (具体的な疑義は findings 側)。
>    - **書き方**: タイトル行で「何が起きているか / どこに意思決定があるか」を述べ、`→` の右側で「**なぜここが論点か**」を述べる。action ("〜を確認しろ") ではなく観点 ("〜が議論点" "〜の設計判断が乗っている" "〜が本丸") で締める。
>    - **file:line は書かない**。領域 / module / 設計の言葉で記述する。
>    - **拾わない**: 具体的なバグや疑義 (findings 側の役割)、「X を grep」「Y を確認」のような action item、mechanical なリネーム・型整形・linter が捕まえるもの。
>    - **フォーマット**: 各 item は番号付き (`  1. ` `  2. ` ...、行頭から半角スペース 2 + `1. `)、item 間に空行 1 行。**2 行目以降は半角スペース 5 つでインデント**して 1 行目本文と縦に揃える。1 行目にタイトル、2 行目に `→` で始めて「なぜここが論点か」。**サブ番号 (`1.` の下にさらに `1.` `2.`) は絶対に使わない** (markdown のネストとして誤解釈される)。例:
>
>      ```
...
>
> **禁止事項** (重要):
> - 具体的な findings (「file.ts:42 でバグ」のような file:line 付き指摘) は書かない。読みながら怪しい箇所を見つけても、**抽象的な観点**に変換する (例: "この判定の境界条件に意図が乗っているか") に留め、「バグである」と断定しない。
> - 「問題なし」「修正不要」等の judgement を書かない。観点の提示に留める。
...
>
> フォーマット規則 (厳守):
> - グループ内の各 item は番号付き (`  1. ` `  2. ` ...、行頭から半角スペース 2 つ + `1. `)、item 間に空行 1 行
> - **2 行目以降は半角スペース 5 つ分インデント**して 1 行目本文と縦に揃える
> - 1 行目: メタ情報 `[Critical 95]` + 問題タイトル (短く、何が壊れるか)
> - 2 行目: file:line を独立した行で (長いパスが本文を圧迫しないように)
> - **2 行目と `根拠:` の間に空行 1 行、`根拠:` と `修正:` は連続 (空行なし)**
> - **`根拠:` `修正:` には絶対に番号を付けない**。サブリスト (`1.` の下にさらに `1.` `2.`) は markdown のネストとして誤解釈されるので一切使わない
