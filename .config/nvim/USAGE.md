...
| キー | 説明 |
|------|------|
| `gd` | DiffviewOpen（git diff を表示） |
| `gh` | DiffviewFileHistory（選択中のファイル/ディレクトリの履歴） |

※ filesystem / git_status どちらのビューでも使用可能
...
| `x` | クリップボードへ切り取り |
| `p` | クリップボードから貼り付け |
| `Y` | パス形式を選んでコピー（絶対パス / 相対パス / ファイル名のみ） |
| `R` | ツリーを更新 |
...
| キー | 説明 |
|------|------|
| `<C-h/j/k/l>` | 隣のウィンドウへ移動（ターミナルモードのまま使用可） |
| `<F7>` / `<C-'>` | フローティングターミナルをトグルして閉じる |
...
| `K` | ホバードキュメント |
| `gd` | 定義へジャンプ |
| `gD` | ソース定義へジャンプ（TS/JS: `.d.ts` をスキップ） |
| `gy` | 型定義へジャンプ |
| `gri` | 実装へジャンプ |
| `grr` / `<Leader>lR` | 参照一覧 |
...
| `<Leader>lA` | ソースアクション（import 整理、未使用コード削除等） |
| `grn` / `<Leader>lr` | リネーム |
| `<Leader>lf` | フォーマット |
...
| キー | 説明 |
|------|------|
| `<Leader>uh` | インレイヒント（バッファ） |
| `<Leader>uH` | インレイヒント（グローバル） |
| `<Leader>uf` | 自動フォーマット（バッファ） |
| `<Leader>uF` | 自動フォーマット（グローバル） |
| `<Leader>uL` | CodeLens |
...
| コマンド | 説明 |
|------|------|
| `:LspTypescriptSourceAction` | source 系コードアクション |
| `:LspTypescriptGoToSourceDefinition` | ソース定義へジャンプ（`gD` と同等） |
